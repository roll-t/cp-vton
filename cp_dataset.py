...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Trước khi chuẩn hóa, mở rộng parse_shape_ori để có 3 kênh
        parse_shape_ori = parse_shape_ori.convert('RGB')

        shape_ori = self.transform(parse_shape_ori)  # [0,1]

        shape = self.transform(parse_shape)  # [0,1]
        phead = torch.from_numpy(parse_head)  # [0,1]
        pcm = torch.from_numpy(parse_cloth)  # [0,1]

        # upper cloth
        im_c = im * pcm + (1 - pcm)  # [0,1]
        im_h = im * phead - (1 - phead)  # [0,1]
