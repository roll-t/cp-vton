...
class FeatureRegression(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x)
        # x = x.view(x.size(0), -1)
        x = x.reshape(x.size(0), -1) # thay đỏi chỗ này từ view sang reshape
        x = self.linear(x)
        x = self.tanh(x)
        return x
...
class GMM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #tham số in-shop-cloth truyền vào 
        self.extractionA = FeatureExtraction(
        22, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d) 
        #tham số truyền vào 22 là sô kênh màu 
        #n_layers=3 là số lớp mạng nơ-ron nhân tạo được sử dụng trong mô hình. 
            #Giá trị này cũng có thể được điều chỉnh để kiểm soát độ phức tạp của mô hình.
        #norm_layer=nn.BatchNorm2d là lớp chuẩn hóa được sử dụng trong mô hình. 
            # Lớp chuẩn hóa giúp cải thiện độ ổn định và hiệu suất của mô hình.
        
        # tham số mark truyền vào
        self.extractionB = FeatureExtraction(
            1, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d) # số 1 đuọc truyền vào là sô channel màu
        
        self.l2norm = FeatureL2Norm()
        self.correlation = FeatureCorrelation()
        self.regression = FeatureRegression(
            input_nc=192, output_dim=2*opt.grid_size**2, use_cuda=False)  # Thay đổi ở đây
        self.gridGen = TpsGridGen(
            opt.fine_height, opt.fine_width, use_cuda=False, grid_size=opt.grid_size)  # Thay đổi ở đây
    ...
    def forward(self, inputA, inputB):
        ...
        correlation = self.correlation(featureA, featureB) #Tính toán độ tương quan giữa hai đặc trưng đã chuẩn hóa

        theta = self.regression(correlation) # Dự đoán lưới biến dạng từ độ tương quan (hồi quy)
        
        grid = self.gridGen(theta) #Tạo lưới biến dạng từ tham số dự đoán 
        # theta là các tham số của lưới biến dạng từ đó ghép ảnh lưới biến dạng lên ảnh người mẫu
        return grid, theta
...
#     torch.save(model.cpu().state_dict(), save_path)  # Thay đổi ở đây
#     model.cuda()
...
def load_checkpoint(model, checkpoint_path):
    if not os.path.exists(checkpoint_path):
        return
    # model.load_state_dict(torch.load(checkpoint_path, map_location=torch.device('cpu')))  # Thay đổi ở đây
    # model.cuda()
    
    # Tải mô hình với map_location=torch.device('cpu')
